...
{
public:
    explicit RustEventProcessorWrapper() : m_processor(event_processor_new())
    {
    }
    void execute(EventPublisher eventPublisher, QByteArray key, QByteArray args) override
    {
        ...
        m_processor->execute(std::make_unique<RustEventPublisher>(std::move(rustPublisherImpl)), key.toStdString(), args.toStdString());
    }
...
std::unique_ptr<EventProcessor> createEventProcessor()
{
    ...
    return std::make_unique<JsonEventAdapter>(std::move(processor));
}
